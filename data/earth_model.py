...
def dam_model():
    thickness_m   = np.array([5.0, 10.0, 15.0, 0.0])
...
def true_group_velocity(model, freqs):
    ...
    g_freqs = 1.0 / res.period
    g_vels = res.velocity * 1000.0
    order = np.argsort(g_freqs)
...
def synth_ccf(freqs, phase_vels, group_vels, distance_m):
    ...
    sig = sig - sig.mean()
    return (sig / (sig.std() + 1e-12)).astype(np.float32)


def make_clean_ccf(distance_m):
    model = dam_model()
    freqs, phase_vels = true_dispersion(model)
    group_vels = true_group_velocity(model, freqs)
    return synth_ccf(freqs, phase_vels, group_vels, distance_m)