...
def load_pairs_with_distance(receivers_path):
...
def build_pairs(pairs, rng,
    ...
    return (np.array(noisy_all, dtype=np.float32),
            ...
            pair_all,      
            np.array(dist_all, dtype=np.float32))# (distance X n_noise_per_ccf, 1001) [if distance is 10, 10 X 5, 1001]

def build_out_of_range_test(distances, rng, levels=(0.02, 0.90),
                            noise_fn=add_noise):
    noisy_parts, clean_parts, level_parts = [], [], []
    for level in levels:
        n, c, l = build_pairs(distances, rng, n_noise_per_ccf=1, noise_fn=noise_fn, fixed_level=level)
